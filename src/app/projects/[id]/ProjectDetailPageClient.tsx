...
function ExpandableStory({ story }: { story: string }) {
  ...
  const visibleStory =
    !isLong || expanded
      ? trimmedStory
      : `${trimmedStory.slice(0, 900).trimEnd()}...`;
  ...
  return (
    <div className="bg-[var(--card)] p-8 rounded-3xl border border-[var(--border)] leading-loose text-lg space-y-4">
      {visibleStory.split("\n").map((para: string, i: number) => (
        <p
          key={i}
          className="text-[var(--text-main)] break-words whitespace-pre-wrap"
        >
          {para}
        </p>
      ))}
...
export default function ProjectDetailPageClient() {
  ...
  const [activeTab, setActiveTab] = useState<"story" | "timeline" | "updates">(
    "story",
  );
  ...
  const loadProject = useCallback(async () => {
    try {
      setLoading(true);
      const data: unknown = await projectService.getProject(projectId);
      // Handle backend response structure { project, milestones }
      if (isProjectResponse(data)) {
        setProject({
          ...data.project,
          milestones: data.milestones || data.project.milestones || [],
        });
      } else if (isRecord(data)) {
        setProject(data as ProjectDetail);
      }
    } catch (error: unknown) {
      setError(getErrorMessage(error, "Project not found"));
    } finally {
      setLoading(false);
    }
  }, [projectId]);
  ...
  const loadDonors = useCallback(async () => {
    try {
      ...
      const items: unknown = await projectService.getCharityDonors(projectId, {
        limit: 50,
      });
      const donorItems = Array.isArray(items)
        ? items
        : isRecord(items) && Array.isArray(items.items)
          ? items.items
          : [];
      setDonors(
        donorItems.filter(isRecord).map((donor) => ({
          id: typeof donor.id === "string" ? donor.id : "",
          donorName:
            typeof donor.donorName === "string" ? donor.donorName : undefined,
          amount:
            typeof donor.amount === "number" || typeof donor.amount === "string"
              ? donor.amount
              : undefined,
        })),
      );
  ...
  const handleSubmitForReview = async () => {
    try {
      ...
      setError(getErrorMessage(error, "Failed to submit project for review"));
    } finally {
  ...
  const daysLeft = project.fundingEndDate
    ? Math.max(
        0,
        Math.ceil(
          (new Date(project.fundingEndDate).getTime() - Date.now()) /
            (1000 * 60 * 60 * 24),
        ),
      )
    : project.deadline
  ...
  const isOwner =
    isAuthenticated &&
    (user?.id || user?._id) &&
    project.creatorId === (user?.id || user?._id);

  const charityProject =
    project.projectType === "CHARITY" || project.type === "CHARITY";
  ...
  const accentShadow = charityProject
            ...
            <div className="lg:col-span-2 space-y-10">
              ...
              <div>
                <div className="flex items-center gap-3 mb-4">
                  <span
                    className={`chip-base chip-compact px-4 py-2 shadow-lg ${charityProject ? "chip-success" : "chip-info"}`}
                  >
                    {charityProject ? "Charity" : "ROI"}
                  </span>
                  <span
                    className={`chip-base chip-compact px-3 py-1.5 ${statusColor}`}
                  >
                    {project.status}
                  </span>
                  {project.category && (
              ...
              <div>
                <div className="flex items-center gap-1 border-b border-[var(--border)] mb-6 overflow-x-auto scrollbar-hide">
                  {(["story", "timeline", "updates"] as const).map((tab) => {
                    ...
                    if (isCharity && (tab === "timeline" || tab === "updates"))
                      return null;
                    ...
                    return (
                      <button
                        ...
                        className={`px-4 sm:px-6 py-3 text-xs sm:text-sm font-black uppercase tracking-widest border-b-2 transition-all -mb-px whitespace-nowrap flex-shrink-0 ${
                          activeTab === tab
                    ...
                    {project.useOfFunds && project.useOfFunds.length > 0 && (
                      <div className="bg-[var(--card)] p-8 rounded-3xl border border-[var(--border)]">
                        ...
                        <div className="space-y-4">
                          {project.useOfFunds.map((item, i: number) => (
                            <div
                              key={i}
            ...
            <div className="space-y-6">
              <div className="sticky top-28 space-y-6">
                ...
                <div className="bg-[var(--card)] rounded-3xl p-8 border border-[var(--border)] shadow-2xl relative overflow-hidden">
                  <div
                    className={`absolute top-0 right-0 w-48 h-48 ${accentGlow} rounded-full blur-3xl -translate-y-24 translate-x-24 pointer-events-none`}
                  />
                          ...
                          {showShareMenu && (
                            <>
                              ...
                              <div className="absolute bottom-full right-0 mb-2 w-56 bg-[var(--card)] border border-[var(--border)] rounded-2xl shadow-2xl overflow-hidden z-50">
                                <p className="px-4 pt-3 pb-1 text-[9px] font-black uppercase tracking-widest text-[var(--text-muted)]">
                                  Share this project
                                </p>
                                ...
                                <button
                                  onClick={() => {
                                    window.open(
                                      `https://twitter.com/intent/tweet?text=${encodeURIComponent(project?.name || "Check out this project!")}&url=${encodeURIComponent(window.location.href)}`,
                                      "_blank",
                                    );
                                ...
                                >
                                  <span className="w-7 h-7 rounded-lg bg-black flex items-center justify-center text-white text-xs font-black flex-shrink-0">
                                    𝕏
                                  </span>
                                  Post on X / Twitter
                                </button>
                                ...
                                <button
                                  onClick={() => {
                                    window.open(
                                      `https://wa.me/?text=${encodeURIComponent((project?.name || "Check this out") + " " + window.location.href)}`,
                                      "_blank",
                                    );
                                    setShowShareMenu(false);
                                  }}
                                ...
                                <button
                                  ...
                                  className="w-full px-4 py-3 flex items-center gap-3 text-sm font-semibold hover:bg-white/5 transition-colors text-left"
                                >
                                  <span className="w-7 h-7 rounded-lg bg-[#1877F2] flex items-center justify-center text-white font-black text-sm flex-shrink-0">
                                    f
                                  </span>
                                  Share on Facebook
                                </button>
                                ...
                                <button
                                  onClick={() => {
                                    window.open(
                                      `https://t.me/share/url?url=${encodeURIComponent(window.location.href)}&text=${encodeURIComponent(project?.name || "")}`,
                                      "_blank",
                                    );
                                    setShowShareMenu(false);
                                  }}
                    ...
                    <div className="pt-4 border-t border-[var(--border)] flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-[var(--primary)] flex items-center justify-center text-white font-black text-sm">
                        {project.creator?.firstName?.[0]?.toUpperCase() ||
                          project.creator?.lastName?.[0]?.toUpperCase() ||
                          project.creator?.email?.[0]?.toUpperCase() ||
                    ...
                    <div className="max-h-44 overflow-y-auto pr-2 space-y-2">
                      {donorsLoading ? (
                        ...
                        donors.map((d) => (
                          <div
                            key={d.id}
                            className="flex items-center justify-between bg-[var(--secondary)] rounded-2xl px-4 py-3"
                          >
                            <div>
                ...
                <div className="p-6 bg-amber-500/5 border border-amber-500/20 rounded-2xl space-y-2">
                  ...
                  <p className="text-xs text-[var(--text-muted)] leading-relaxed">
                    Keibo facilitates fundraising but doesn&apos;t guarantee
                    project delivery or outcomes. Donations are non-refundable
                    unless explicitly stated. Contribute what you can afford.
                  </p>
                </div>
              </div>
            </div>
