...
export default function SettingsPage() {
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      ...
      if (firstName !== (user?.profile?.firstName || user?.firstName))
        updateData.firstName = firstName;
      if (lastName !== (user?.profile?.lastName || user?.lastName))
              ...
              <div className="flex gap-1 overflow-x-auto scrollbar-hide bg-[var(--card)] border border-[var(--border)] rounded-2xl p-1.5">
                {tabs.map((tab) => (
                  <button
                    ...
                    className={`flex items-center gap-2 px-3 py-2.5 rounded-xl text-xs font-bold transition-all whitespace-nowrap flex-shrink-0 ${
                      activeTab === tab.id
                ...
                {activeTab === "appearance" && (
                  <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-8 space-y-6">
                    <div>
                      ...
                      <p className="text-sm text-[var(--text-muted)] mt-1">
                        Choose how Keibo looks for you.
                      </p>
                    </div>
                        ...
                        {(
                          [
                            ...
                            },
                            {
                              ...
                              preview: "bg-[#0d1828] border-[#1e2d45]",
                            },
                            {
                              id: "system",
                              label: "System",
                  ...
                  <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-8 space-y-6">
                      ...
                      <div className="flex items-center justify-between p-5 rounded-2xl border border-[var(--border)] bg-[var(--secondary)]">
                        <div className="space-y-0.5">
                          ...
                          <p className="text-xs text-[var(--text-muted)]">
                            Update your login password from the Profile tab.
                          </p>
                        </div>
                        <button
                      ...
                      <div className="p-5 rounded-2xl border border-[var(--border)] bg-[var(--secondary)] space-y-4">
                        <div className="flex items-center justify-between gap-4">
                          <div className="space-y-0.5">
                            ...
                            <p className="text-xs text-[var(--text-muted)]">
                              {mfaEnabled
                                ? "A second verification step is required when signing in."
                                : "Add an authenticator app or email code to protect account access."}
                            </p>
                          </div>
                          <span
                            className={`chip-base ${mfaEnabled ? "chip-success" : "chip-muted"} flex-shrink-0`}
                          >
                            {mfaEnabled ? (
                              <CheckCircle size={13} />
                            ) : (
                        ...
                        {!emailMfaEnabled && mfaEmailSetupStarted && (
                          <div className="space-y-3 rounded-2xl border border-[var(--border)] bg-[var(--card)] p-4">
                            <p className="text-sm text-[var(--text-muted)]">
                              We sent a 6-digit MFA code to your account email.
                        ...
                        <p className="text-xs text-[var(--text-muted)]">
                          Supported now: authenticator-app codes and email
                          codes. SMS requires a configured SMS provider before
                          it can be enabled.
                        </p>
                      </div>
                              ...
                              <div className="space-y-0.5">
                                ...
                                <p className="text-xs text-[var(--text-muted)]">
                                  {kycStatus === "VERIFIED"
                                    ...
                                    : kycStatus === "PENDING"
                                      ? "Verification is under review — we will notify you."
                                      : kycStatus === "REJECTED"
                                        ? "Your submission was not approved. Please re-verify."
                    ...
                    <div className="p-5 rounded-2xl border border-rose-500/20 bg-rose-500/5 flex items-center justify-between">
                      <div className="space-y-0.5">
                        ...
                        <p className="text-xs text-[var(--text-muted)]">
                          Permanently removes your account and all data. This
                          cannot be undone.
                        </p>
                      </div>
                      <button
                    ...
                    <div className="p-4 bg-[var(--secondary)] border border-[var(--border)] rounded-xl flex items-start gap-3">
                      ...
                      <p className="text-xs text-[var(--text-muted)] leading-relaxed">
                        ...
                        <button
                          ...
                          className="font-semibold text-[var(--text-main)] underline"
                        >
                          support page
                        </button>{" "}
                        for the available contact channel.
                      </p>
                    </div>
                  </div>
                )}
