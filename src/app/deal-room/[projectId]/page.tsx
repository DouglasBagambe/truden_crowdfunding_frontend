...
const TIERS = [
  ...
  },
  {
    ...
    label: "Starter",
    minInvestment: 1_000,
    description: "Financial statements & projections",
          ...
          <div className="flex-1 overflow-auto p-4 min-h-[400px]">
            {isImage ? (
              ...
              <iframe
                src={url}
                title={filename}
                className="w-full h-[60vh] rounded-xl border border-[var(--border)]"
              />
            ) : isVideo ? (
              <video
                src={url}
...
function UploadPanel({
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    try {
      ...
      showError("Upload failed", errorMessage(error, "Upload failed"));
    } finally {
      setUploading(false);
    }
  };
  ...
  return (
    <div className="bg-[var(--card)] rounded-3xl border border-[var(--border)] p-6 space-y-4">
      ...
      <div className="flex items-center gap-3">
        <label className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-widest shrink-0">
          Category
        </label>
        <select
          value={category}
...
export default function DealRoomProjectPage() {
  const dealRoomEnabled = process.env.NEXT_PUBLIC_DEAL_ROOM_ENABLED === "true";
  ...
  const load = async () => {
    if (!dealRoomEnabled) return;
    try {
      setLoading(true);
      const [filesRes] = await Promise.allSettled([
        dealRoomService.listProjectFiles(projectId),
      ]);
      if (filesRes.status === "fulfilled") {
        setItems(filesRes.value.items || []);
      }
    } catch (error: unknown) {
      showError("Failed to load", errorMessage(error, "Failed to load"));
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    if (!dealRoomEnabled || authLoading) return;
    if (!isAuthenticated) {
  ...
  }, [authLoading, dealRoomEnabled, isAuthenticated, projectId]);

  useEffect(() => {
    if (dealRoomEnabled && !roiLoading && !hasRoiAccess) {
      showError(
        "Access restricted",
        "The deal room is available to internal ROI users only.",
      );
      router.replace("/explore");
    }
  }, [dealRoomEnabled, hasRoiAccess, roiLoading, router, showError]);
  ...
  const openPreview = async (doc: DealRoomFileItem) => {
    try {
      ...
      showError("Preview failed", errorMessage(error, "Preview failed"));
    } finally {
      setActingOn(null);
    }
  };
  ...
  const downloadFile = async (doc: DealRoomFileItem) => {
    try {
      ...
      showError("Download failed", errorMessage(error, "Download failed"));
    } finally {
      setActingOn(null);
    }
  };
  ...
  const filtered = useMemo(
    () =>
      items.filter(
        (d) =>
          activeCategory === "ALL" ||
          (d.category || "GENERAL") === activeCategory,
      ),
    [items, activeCategory],
  );

  const isAdmin = (user?.roles || []).some((r: string) =>
    ["ADMIN", "admin"].includes(r),
  );
      ...
      <div className="min-h-screen bg-[var(--background)] pt-24">
        ...
        <main className="mx-auto max-w-2xl px-6 py-24 text-center">
          ...
          <p className="mt-3 text-[var(--text-muted)]">
            Durable encrypted document storage and signed-download delivery are
      ...
      <main className="max-w-7xl mx-auto px-6 pb-24">
        ...
        <div className="flex items-start justify-between gap-6 mb-8">
          <div className="flex items-start gap-4">
            <Link
              href={`/projects/${projectId}`}
              className="p-2 rounded-xl hover:bg-[var(--secondary)] transition-colors mt-1"
            >
            ...
            <div>
              ...
              <p className="text-sm text-[var(--text-muted)] font-medium">
                Confidential investor documents. Access based on investment
                tier.
              </p>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-[var(--card)] rounded-3xl border border-[var(--border)] p-5 space-y-4">
              ...
              {TIERS.map((t) => (
                <div
                  key={t.tier}
                  className={`p-3 rounded-2xl border transition-all ${t.tier <= userTier ? `${t.bg} border-current/20` : "border-[var(--border)] opacity-40"}`}
                >
                  <div className="flex items-center justify-between mb-1">
                    <span
                      className={`text-xs font-black ${t.tier <= userTier ? t.color : "text-[var(--text-muted)]"}`}
                    >
                      Tier {t.tier} — {t.label}
                    </span>
                    {t.tier <= userTier ? (
          ...
          <div className="lg:col-span-3 space-y-6">
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        <button
                          ...
                          title={
                            !doc.canDownload
                              ? "Upgrade your investment tier to download"
                              : "Download"
                          }
            ...
            <div className="flex items-start gap-3 p-4 rounded-2xl bg-blue-500/5 border border-blue-500/20">
              ...
              <p className="text-xs text-[var(--text-muted)] font-medium">
                Documents are stored on IPFS for tamper-proof, decentralized
                access. Your investment tier determines which documents you can
                view and download.
              </p>
            </div>
          </div>
