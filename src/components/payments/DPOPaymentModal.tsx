...
function toMinorUnits(value: string, currency: string): string {
  ...
  if (fraction.length > scale && /[1-9]/.test(fraction.slice(scale))) {
    throw new Error("Amount has too many decimal places for this currency.");
  }
  return (
    BigInt(match[1]) * BigInt(10) ** BigInt(scale) +
...
function getDisplayName(user: unknown): string {
  ...
  const firstName =
    currentUser?.profile?.firstName || currentUser?.firstName || "";
  const lastName =
    currentUser?.profile?.lastName || currentUser?.lastName || "";
...
function resolveErrorLabel(errorText: string): {
  label: string;
  icon: React.ReactNode;
} {
  const lower = errorText.toLowerCase();
  if (
    lower.includes("not yet been provisioned") ||
    lower.includes("provisioned on-chain")
  ) {
    return {
...
export default function DPOPaymentModal({
  isOpen,
  onClose,
  project,
}: DPOPaymentModalProps) {
  ...
  const investmentQuickAmounts = useMemo(
        ...
        if (!cancelled) {
          ...
          const message = normalizeErrorMessage(
            (
              ...
              (quoteError as { response?: { data?: { error?: unknown } } })
                ?.response?.data?.error ??
              quoteError,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const intent = await createPaymentIntent({
        projectId,
        amountMinor: toMinorUnits(String(quote.grossAmount), currency),
        currency,
          ...
          >
            <div className="flex w-full max-w-md max-h-[calc(100dvh-2rem)] flex-col overflow-hidden rounded-3xl border border-[var(--border)] bg-[var(--card)] shadow-2xl sm:max-h-[calc(100dvh-3rem)]">
              ...
              <div className="min-h-0 flex-1 space-y-5 overflow-y-auto overscroll-contain px-6 py-6">
                ...
                {isROI && (
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-[var(--text-muted)]">
                      This self-custodial address only receives an investment
                      receipt after backend eligibility and settlement checks
                      pass.
                    </p>
                  </div>
                )}
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-[var(--text-muted)]">
                    {isCharity ? "Donation Amount" : "Investment Amount"} (
                    {currency})
                  </label>
                  <input
                    type="number"
                  ...
                  <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] px-4 py-4 text-sm">
                    <div className="flex items-center justify-between py-1 text-[var(--text-muted)]">
                      ...
                      <span className="font-bold text-[var(--text-main)]">
                        {quote.currency}{" "}
                        {quote.projectNetAmount.toLocaleString()}
                      </span>
                    </div>
                    <div className="flex items-center justify-between py-1 text-[var(--text-muted)]">
                      <span>Provider fee</span>
                      <span>
                        {quote.currency} {quote.dpoFee.toLocaleString()}
                      </span>
                    </div>
                    <div className="flex items-center justify-between py-1 text-[var(--text-muted)]">
                      <span>KEIBO payment fee</span>
                      <span>
                        {quote.currency} {quote.keiboFee.toLocaleString()}
                      </span>
                    </div>
                    <div className="flex items-center justify-between py-1 text-[var(--text-muted)]">
                      <span>Taxes</span>
                      <span>
                        {quote.currency} {quote.dpoVat.toLocaleString()}
                      </span>
                    </div>
                    <div className="mt-2 flex items-center justify-between border-t border-[var(--border)] pt-3 text-[var(--text-main)]">
                      ...
                      <span className="text-base font-black">
                        {quote.currency} {quote.grossAmount.toLocaleString()}
                      </span>
                    </div>
                  </div>
                )}
                  ...
                  >
                    ...
                    <div className="space-y-0.5">
                      ...
                      <p className="font-medium leading-5">
                        ROI provisioning checks are bypassed on this platform
                        instance.
                        {!quote.roi.nftMintingEnabled &&
                          " NFT minting is also disabled."}{" "}
                        Your investment will be recorded but may not be backed
                        by an on-chain NFT until production provisioning is
                        complete.
                      </p>
                    </div>
                  </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {(isCharity ? donorQuickAmounts : investmentQuickAmounts).map(
                    (quickAmount) => (
                      <button
                        key={quickAmount}
                        type="button"
                ...
                {error &&
                  (() => {
                    ...
                    const isProvisioningError =
                      ...
                      error
                        .toLowerCase()
                        .includes("not yet been provisioned") ||
                      error
                        .toLowerCase()
                        .includes("provisioning previously failed");
                          ...
                          <div className="space-y-1">
                            ...
                            {isProvisioningError && (
                              <p className="text-xs text-amber-700 mt-1">
                                This project is approved but its blockchain
                                record has not been set up yet. Contact the
                                platform admin to complete the setup.
                              </p>
                            )}
                          </div>
                        </div>
